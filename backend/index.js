...
// API: Gửi trạng thái nút từ frontend → ESP32 (gửi kèm ngưỡng)
...
// API: Cập nhật ngưỡng cảnh báo (giữ nguyên gửi qua ws như cũ)
app.post('/api/thresholds', (req, res) => {
    ...
    // Gửi qua WebSocket cho ESP32 nếu đang kết nối
    if (esp32Socket && esp32Socket.readyState === WebSocket.OPEN) {
...
// Hàm chuyển đổi ISO sang định dạng MySQL DATETIME (giờ Việt Nam)
function toMySQLDatetimeVN(date) {
        ...
        ws.on('message', (message) => {
            try {
                ...
                if (
                    ...
                    const timestamp = toMySQLDatetimeVN(new Date());
                ...
                (err2, alertRows) => {
                    ...
                    db.query(
                        ...
                        (err3, actRows) => {
                            ...
                            // Sửa alertCounts: ép kiểu về số và thay null/undefined bằng 0
                            const alertCounts = {
                                low: Number(alertRows[0].low) || 0,
                                medium: Number(alertRows[0].medium) || 0,
                                high: Number(alertRows[0].high) || 0,
                            };
