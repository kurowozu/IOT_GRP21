  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      try {
        ...
        if (JSON.stringify(sensorRes.data) !== JSON.stringify(sensorData)) setSensorData(sensorRes.data);
        if (JSON.stringify(buttonRes.data) !== JSON.stringify(buttonStates)) setButtonStates(buttonRes.data);
        if (JSON.stringify(historyRes.data) !== JSON.stringify(historyLog)) setHistoryLog(historyRes.data);
      } catch (err) {
    ...
    fetchAll();
    interval = setInterval(fetchAll, 5000); // 5 giây cập nhật một lần
    return () => clearInterval(interval);
    // eslint-disable-next-line
  }, [sensorData, buttonStates, historyLog]);
