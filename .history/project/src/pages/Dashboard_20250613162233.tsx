...
// Thông tin hệ thống (có thể lấy động từ backend nếu muốn)
const systemInfo = {
  version: '1.2.0',
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-slate-800/60 border border-slate-600/50 rounded-2xl p-6 shadow-2xl flex flex-col gap-4">
            <h2 className="text-2xl font-bold text-white mb-2">Thông tin hệ thống</h2>
            <div className="flex flex-col gap-2 text-slate-200 text-base">
              <div className="flex justify-between">
                <span>Phiên bản phần mềm</span>
                <span className="font-semibold text-emerald-400">{systemInfo.version}</span>
              </div>
              <div className="flex justify-between">
                <span>Thời gian hoạt động</span>
                <span className="font-semibold text-cyan-400">{systemInfo.uptime}</span>
              </div>
              <div className="flex justify-between">
                <span>Trạng thái kết nối</span>
                <span className={`font-semibold ${systemInfo.status === 'Đã kết nối' ? 'text-emerald-400' : 'text-rose-400'}`}>{systemInfo.status}</span>
              </div>
              <div className="flex justify-between">
                <span>Địa chỉ IP</span>
                <span className="font-semibold text-amber-400">{systemInfo.ip}</span>
              </div>
              <div className="flex justify-between">
                <span>Lần cập nhật cuối</span>
                <span className="font-semibold text-slate-300">{systemInfo.lastUpdate}</span>
              </div>
            </div>
          </div>
          {/* Bên phải là block cài đặt ngưỡng, map, đồng hồ, thời tiết */}
          <div>
            {/* Cài đặt ngưỡng */}
            ...
            {/* --- Đưa GoogleMap, Clock, Weather vào đây --- */}
            <div className="flex flex-col gap-3 items-end min-w-[260px] w-[260px]">
              <GoogleMap />
              <div className="flex flex-row gap-3 w-full justify-between mt-2">
                <Clock />
                <Weather />
              </div>
            </div>
...
};  );    </div>      </div>            <div className="mt-6 pt-4 border-t border-slate-600">
