...
import { Waves, Zap, AlertTriangle, CheckCircle, Activity } from 'lucide-react';
...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    // Lấy trạng thái button từ backend khi tải trang
                  ...
                  <div className="w-full bg-slate-700 rounded-full h-4 overflow-hidden">
                    <div 
                      className={`h-4 rounded-full transition-all duration-500 ${
                        sensorData.sensor1Distance < 15 ? 'bg-gradient-to-r from-rose-500 to-red-500' :
                        sensorData.sensor1Distance < 25 ? 'bg-gradient-to-r from-amber-500 to-orange-500' : 
              ...
              <div className="bg-gradient-to-br from-slate-700/50 to-slate-800/50 rounded-xl p-6 border border-slate-600/30">
                ...
                <div className="mb-4">
                  ...
                  <div className="w-full bg-slate-700 rounded-full h-4 overflow-hidden">
                    <div 
                      className={`h-4 rounded-full transition-all duration-500 ${
                        sensorData.sensor2Distance < 15 ? 'bg-gradient-to-r from-rose-500 to-red-500' :
                        sensorData.sensor2Distance < 25 ? 'bg-gradient-to-r from-amber-500 to-orange-500' : 
                        'bg-gradient-to-r from-emerald-500 to-teal-500'
                      } shadow-lg`}
          ...
          <div className="bg-slate-800/60 backdrop-blur-sm border border-slate-600/50 rounded-2xl p-6 shadow-2xl">
            ...
            <div className="mt-6 pt-4 border-t border-slate-600">
              <div className="text-sm text-slate-400 text-center">
                Nút đang nhấn: <span className="text-emerald-400 font-semibold">{buttons.filter(b => b.isPressed).length}/8</span>
              </div>
            </div>
          </div>
        </div>
